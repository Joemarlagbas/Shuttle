...
// Fetch driver info
$stmt = $conn->prepare("SELECT username, approved, tracking_status FROM driver_accounts WHERE id=?");
...
$shuttle = $shuttle_stmt->get_result()->fetch_assoc();
...
<button class="btn btn-danger btn-start mt-3" id="stopBtn" disabled>Stop GPS Tracking</button>
...
startBtn.addEventListener('click', function() {
    ...
    navigator.geolocation.getCurrentPosition(function(pos) {
        ...
        // Update tracking status in DB
        fetch('update_tracking_status.php', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({ driver_id: driverId, status: 1 })
        });
...
stopBtn.addEventListener('click', function() {
    if (watchId !== null) {
        ...
        watchId = null;
        statusBox.innerText = "❌ Tracking stopped.";
        ...
        fetch('update_tracking_status.php', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({ driver_id: driverId, status: 0 })
        });
    }
});
...
function sendLocation(position) {
    ...
    fetch('save_location.php', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({ shuttle_id: shuttleId, driver_id: driverId, lat, lng })
    })
    .then(r=>r.json())
    .then(data => {
