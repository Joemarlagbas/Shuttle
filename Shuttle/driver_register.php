...
if (isset($_POST['register'])) {
    ...
    if ($password !== $confirm) {
        ...
        $check = $conn->prepare("SELECT id FROM driver_accounts WHERE username=?");
        $check->bind_param("s", $us);
        $check->execute();
        $check->store_result();
        ...
        if ($check->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO driver_accounts (username,password,approved) VALUES (?,?,0)");
